...
export function calculateProjections(
  ...
  const extraAnnualPayment = Math.max(
    0,
    inputs.yearlyInvestment - annualMortgageCosts
  );
  ...
  for (let year = 1; year <= maxYears; year++) {
    ...
    stockValue =
      stockValue * (1 + inputs.stockAnnualReturn / 100) + netStockInvestment;
    ...
    propertyValue =
      inputs.houseCost * Math.pow(1 + inputs.houseGrowthRate / 100, year);
    ...
    const stockTotalCost =
      cumulativeRent +
      ((inputs.initialNetWorth * inputs.stockAnnualReturn) / 100) * year; // Opportunity cost
    const propertyTotalCost = interestPaid + cumulativeOwnersCosts;

    projections.push({
      year,
      stockValue,
      stockCumulativeInvestment:
        inputs.initialNetWorth + netStockInvestment * year,
